...
class waveform():
    ...
    def calculate(self,a):
        ...
        Fenv=concatenate([Fir,Fir,Fir,[0]])
        ...
        #                L2fftout.set_data(    f/os, abs(S[f+N//2]))
            ...
            if i==1: L2fftpoints.set_data(arange(-len(SS)//2,len(SS)//2),real(SS))
        ...
        textMER.set_text('MER est.=%0.1f'%(10*log10(sum(abs(s3-sr[i])**2)/sum(abs(ww[i])**2)+1e-20)))
